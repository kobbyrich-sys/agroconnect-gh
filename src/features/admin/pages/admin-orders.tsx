        ...
        <>
          ...
          <div className="space-y-3 sm:hidden">
            {orders.map((order: any) => (
              <div key={order.id} className="rounded-lg border border-earth-200 bg-white p-4">
                <div className="flex items-center justify-between mb-2">
                  <Link to={`/orders/${order.id}`} className="text-agro-600 hover:text-agro-700 font-mono text-xs font-medium">
                    #{order.id.slice(0, 8)}
                  </Link>
                  <span className="text-xs text-earth-400">{new Date(order.created_at).toLocaleDateString()}</span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                  <div><span className="text-earth-500 text-xs">Buyer:</span><p className="text-earth-700 truncate">{order.buyer?.full_name || 'Unknown'}</p></div>
                  <div><span className="text-earth-500 text-xs">Seller:</span><p className="text-earth-700 truncate">{order.seller?.full_name || 'Unknown'}</p></div>
                  <div><span className="text-earth-500 text-xs">Total:</span><p className="font-medium">GH₵ {Number(order.total).toFixed(2)}</p></div>
                  <div><span className="text-earth-500 text-xs">Status:</span>
                  ...
                  <div className="col-span-2"><span className="text-earth-500 text-xs">Payment:</span>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ml-1 ${order.payment_status === 'pending' ? 'bg-yellow-100 text-yellow-800' : order.payment_status === 'awaiting_payment' ? 'bg-gold-100 text-gold-800' : order.payment_status === 'paid' || order.payment_status === 'escrow_held' ? 'bg-blue-100 text-blue-800' : order.payment_status === 'escrow_released' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{PAYMENT_LABELS[order.payment_status] || order.payment_status}</span>
                  </div>
                </div>
                {renderActions(order)}
              </div>
          ...
          {/* Desktop table */}
          <div className="hidden sm:block overflow-x-auto rounded-lg border border-earth-200 bg-white">
